{% macro init_raw_spotify_tables() %}
  {% if target.type == 'duckdb' %}
    ...
    {% do run_query("create table if not exists raw.musicbrainz_isrc_failures (isrc varchar, error_status_code varchar, error_message varchar, last_failed_at varchar, error_404_count integer)") %}
    {% do run_query("alter table raw.musicbrainz_isrc_failures add column if not exists error_404_count integer") %}
    {% do run_query("update raw.musicbrainz_isrc_failures set error_404_count = 0 where error_404_count is null") %}
