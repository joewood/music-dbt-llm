...
def _extract_status_code(exc: Exception) -> str:
    """Best-effort extraction of HTTP status code from MusicBrainz client exceptions."""
    text = str(exc)
...
def run_enrichment(
    input_csv: str = "exports/musicbrainz/enrichment_queue.csv",
    output_dir: str = "exports/musicbrainz/results",
    max_unenriched: int = 1000,
    ...
    isrc_to_tracks = read_enrichment_queue(input_csv_path, max_unenriched)
    ...
    failed_isrc_rows: Dict[str, dict] = {}
    ...
    candidates_csv = output_dir_path / "musicbrainz_isrc_candidates.csv"
    recording_payloads_csv = output_dir_path / "musicbrainz_recording_payloads.csv"
    work_payloads_csv = output_dir_path / "musicbrainz_work_payloads.csv"
    failed_isrcs_csv = output_dir_path / "musicbrainz_isrc_failures.csv"
    ...
    failed_isrc_count = write_csv(
        failed_isrcs_csv,
        ["isrc", "error_status_code", "error_message", "last_failed_at"],
        failed_isrc_rows.values(),
    )
        ...
        json.dumps(
            {
                ...
                "rows": {
                    ...
                    "musicbrainz_isrc_failures": failed_isrc_count,
                },
    ...
    print(
        ...
        f"{failed_isrc_count} failed ISRC rows."
    )
